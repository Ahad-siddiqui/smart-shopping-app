...
export default function AdminPendingAdsScreen({ navigation }) {
  ...
  const handleApprove = async (item) => {
    try {
      await adminService.approveAd(item._id);
  ...
  const submitReject = async (item) => {
    ...
    try {
      await adminService.rejectAd(item._id, rejectReason.trim());
      setState((prev) => ({ ...prev, items: prev.items.filter((p) => p._id !== item._id) }));
      setRejectingId(null);
      setRejectReason('');
    } catch (err) {
    ...
    <FlatList
      ...
      ListEmptyComponent={
        <View style={styles.empty}>
          <Text style={styles.emptyText}>No ads pending review.</Text>
        </View>
      }
      renderItem={({ item }) => (
        <View style={styles.card}>
          <TouchableOpacity style={styles.cardBody} onPress={() => navigation.navigate('ProductDetails', { id: item._id })}>
            <Image source={{ uri: item.images?.[0]?.url }} style={styles.thumb} />
            <View style={{ flex: 1 }}>
              <Text style={styles.title} numberOfLines={1}>{item.title}</Text>
              <Text style={styles.price}>{CURRENCY_SYMBOL}{Number(item.price).toLocaleString()}</Text>
...
const styles = StyleSheet.create({
  ...
  price: { fontSize: 13, fontWeight: '700', color: '#0f766e', marginTop: 2 },
  owner: { fontSize: 11, color: '#a3a3a3', marginTop: 2 },
